...
class Handler:
        ...
        def on_cholesk_radiobutton_toggled(self,button):
                active=button.get_active()
                if active:
                        self.app.saisir_operandes("CHOLESKY","MATRICE","Factorisation par la methode de Cholesky d'une matrice")
        ...
        def on_edit_base_button1_clicked(self,button):
                #base=[['v1',4,True],['v2',2,True],['v3',3,False],['v4',4,False],['v6',4,False],['v7',4,False],['v5',4,False]]
                base=[]
                for name in self.app.matrix_list:
                        n=self.app.matrix_list[name][1]
                        p=self.app.matrix_list[name][2]
                        if n==1 and p>=2:
                                base.append([name,p,False])
                        elif n>=2 and p==1:
                                base.append([name,n,False])

                dialog=BaseEditionDialog(base=base,parent=self.app.window)
                response=dialog.run()
